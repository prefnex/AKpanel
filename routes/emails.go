...
func registerEmailsRoutes(emailController *controllers.EmailController) {
	...
	facades.Route().Post("/api/emails/queue/retry", emailController.RetryQueue)
	facades.Route().Get("/api/emails/queue/content", emailController.QueueMessageContent)
	facades.Route().Get("/api/emails/deliveries", emailController.Deliveries)
	facades.Route().Get("/api/emails/deliveries/detail", emailController.DeliveryDetail)
	facades.Route().Get("/api/emails/diagnostics", emailController.MailDiagnostics)
